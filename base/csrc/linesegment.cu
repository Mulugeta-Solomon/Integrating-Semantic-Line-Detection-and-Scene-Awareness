...
__global__ void encode_kernel(const int nthreads, const float* lines,
                           ...
                           const int height, const int width, float* map, float* annotation,
                           bool* label, float* tmap, const float* line_annotations)
{
    CUDA_1D_KERNEL_LOOP(index, nthreads){
        ...
        int annotation_index = h*width + w;
        ...
        for(int i = 0; i < num; ++i) {
            ...
            float curr_annotation = line_annotations[i];
            ...
            if (dis < min_dis) {
                ...
                annotation[annotation_index] = curr_annotation;
...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor> lsencode_cuda(
    ...
    const int width,
    const int num_lines,
    const at::Tensor& line_annotations)

{
    auto map = at::zeros({6,height,width}, lines.options());
    auto annotation = at::zeros({1,height,width}, lines.options());
    ...
    float* annotation_data = annotation.data<float>();
    ...
    encode_kernel<<<CUDA_GET_BLOCKS(nthreads), CUDA_NUM_THREADS >>>(
            ...
            label_data,
            tmap_data, 
            line_annotations.data<float>());

    // THCudaCheck(cudaGetLastError());

    return std::make_tuple(map, annotation, label, tmap);
}